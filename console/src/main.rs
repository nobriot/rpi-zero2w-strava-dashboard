  ...
  let request = match cmd {
    ...
    "wifi" => match arg1 {
      ...
      "add" => {
        ...
        Some(protocol::Request::WifiAdd { ssid: arg2.to_string(),
                                          password })
      },
      "forget" => {
        if arg2.is_empty() {
        ...
        Some(protocol::Request::WifiForget { ssid: arg2.to_string() })
      },
      _ => {
    ...
    Ok(resp) if resp.ok => {
      if let Some(protocol::ResponseData::WifiNetworks(networks)) = &resp.data {
        ...
        for (i, net) in networks.iter().enumerate() {
          eprintln!("  {}. {} ({} dBm) [{}]", i + 1, net.ssid, net.signal, net.security);
        }
        ...
        eprint!("Connecting... ");
        match conn.send(&protocol::Request::WifiAdd { ssid: ssid.clone(), password }) {
    ...
    match &resp.data {
      ...
      Some(protocol::ResponseData::SystemStatus(status)) => {
        eprintln!("  WiFi:    {} {}",
                  ...
                  status.wifi_ssid.as_deref().unwrap_or(if status.wifi_connected {
                                                          "Connected"
                                                        } else {
                                                          "Not connected"
                                                        }));
        eprintln!("  Config:  {}",
                  if status.config_valid { "✓ Valid" } else { "✗ Missing or invalid" });
        eprintln!("  Auth:    {}",
                  if status.auth_valid { "✓ Valid" } else { "✗ Invalid or missing token" });
