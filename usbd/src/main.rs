...
fn run_session(device: &str) -> Result<(), String> {
  ...
  for line in reader.lines() {
    ...
    write_file.write_all(out.as_bytes()).map_err(|e| format!("Write error: {e}"))?;
    write_file.flush().map_err(|e| format!("Flush error: {e}"))?;
  }
...
fn config_path() -> String {
  let dir = dirs::config_dir().unwrap_or_else(|| std::path::PathBuf::from(".config"))
...
fn system_status() -> protocol::Response {
  ...
  protocol::Response::ok_data(protocol::ResponseData::SystemStatus(protocol::SystemStatus {
