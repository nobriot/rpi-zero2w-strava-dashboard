...
mod tests {
  ...
  fn test_decode_polyline() {
    // Standard Google example: encodes (38.5, -120.2), (40.7, -120.95), (43.252,
    // -126.453)
  ...
  fn test_sport_type_run_variants() {
    for sport_type in ["Run", "TrailRun", "VirtualRun"] {
      let json = format!(r#"{{"id":1,"sport_type":"{}","distance":5000.0}}"#, sport_type);
  ...
  fn test_sport_type_ride_variants() {
    for sport_type in
      ["Ride", "MountainBikeRide", "GravelRide", "VirtualRide", "Handcycle", "Velomobile"]
    {
      let json = format!(r#"{{"id":1,"sport_type":"{}","distance":20000.0}}"#, sport_type);
      let a: SummaryActivity = serde_json::from_str(&json).unwrap();
  ...
  fn test_missing_sport_type_falls_back_to_type() {
    ...
    let a: SummaryActivity = serde_json::from_str(&json).unwrap();
    assert_eq!(a.sport(), Some(SportType::Run), "should fall back to deprecated type field");
  }
